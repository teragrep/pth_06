...
public class HdfsDBClient {
    ...
    public HdfsDBClient(Config config, String topicsRegexString) throws IOException {
        ...
        if ("kerberos".equals(config.hdfsConfig.kerberosAuthentication)) {
            ...
            System.setProperty("java.security.krb5.realm", config.hdfsConfig.kerberosRealm);
            System.setProperty("java.security.krb5.kdc", config.hdfsConfig.kerberosKdc);

            conf = new Configuration();

            // enable kerberus
            conf.set("hadoop.security.authentication", config.hdfsConfig.kerberosAuthentication);
            conf.set("hadoop.security.authorization", config.hdfsConfig.kerberosAuthorization);
            conf.set("hadoop.kerberos.keytab.login.autorenewal.enabled", config.hdfsConfig.kerberosAutorenewal);
            ...
            conf.set("dfs.client.use.datanode.hostname", config.hdfsConfig.UseHdfsHostname);

            // server principal, the kerberos principle that the namenode is using
            conf.set("dfs.namenode.kerberos.principal.pattern", config.hdfsConfig.kerberosPrincipalPattern);

            conf.set("dfs.data.transfer.protection", config.hdfsConfig.hdfsTransferProtection);
            conf.set("dfs.encrypt.data.transfer.cipher.suites", config.hdfsConfig.hdfsCipherSuites);

            // set usergroup stuff
            UserGroupInformation.setConfiguration(conf);
            UserGroupInformation
                    .loginUserFromKeytab(config.hdfsConfig.kerberosKeytabUser, config.hdfsConfig.kerberosKeytabPath);
