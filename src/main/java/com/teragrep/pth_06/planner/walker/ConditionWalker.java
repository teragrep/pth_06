...
public final class ConditionWalker extends XmlWalker<Condition> {
    ...
    public Condition fromString(String inXml, boolean streamQueryArg)
        ...
        final Condition walkerCondition = fromString(inXml);
        final Condition returnedCondition;
        if (!filterlessSearch.isStub()) {
            ...
            returnedCondition = withoutFiltersCondition.and(walkerCondition);
        }
        else {
            returnedCondition = walkerCondition;
        }
    ...
    Condition emitElem(final Element current) {
        ...
        if (elementCondition.isBloomSearchCondition() && !filterlessSearch.isStub()) {
            throw new RuntimeException("Search terms are not allowed when <bloom.withoutFilters> option is enabled");
        }
