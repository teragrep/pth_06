...
class ArchiveMicroBatchInputPartitionReader implements InputPartitionReader<InternalRow> {
    ...
    public ArchiveMicroBatchInputPartitionReader(String S3endPoint,
                                                 String S3identity,
                                                 String S3credential,
    ...
    public boolean next() throws IOException {
        ...
        while (taskObjectList.size() > 0 && !rv) {
            ...
            if (rowConverter == null) {
                ...
                rowConverter = new RowConverter(
                        auditPlugin,
                        s3client,
						taskObjectList.getFirst().getId(),
                        taskObjectList.getFirst().getBucket(),
                        taskObjectList.getFirst().getPath(),
                        taskObjectList.getFirst().getDirectory(),
                        taskObjectList.getFirst().getStream(),
                        taskObjectList.getFirst().getHost(),
                        skipNonRFC5424Files
                );
                rowConverter.open();
            }
