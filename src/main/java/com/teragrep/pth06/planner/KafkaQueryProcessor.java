...
public class KafkaQueryProcessor implements KafkaQuery {
    ...
    public KafkaQueryProcessor(Config config) {
        Map<String, Object> executorKafkaProperties = config.getKafkaDriverOpts();
        ...
        if (config.getQuery() != null) {
            try {
                ...
                topicsRegexString = parser.fromString(config.getQuery());
            } catch (Exception ex) {
                ex.printStackTrace();
                throw new RuntimeException("KafkaQueryProcessor problems when construction Query conditions query:" + config.getQuery() + " exception:" + ex);
            }
        }
