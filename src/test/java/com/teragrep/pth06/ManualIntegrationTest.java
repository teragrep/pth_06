 ...
 */
        ...
        import com.teragrep.pth06.planner.MockDBData;
        import com.teragrep.pth06.task.s3.MockS3;
        import com.teragrep.pth06.task.s3.Pth06S3Client;
        import com.teragrep.pth06.task.s3.RowConverter;
...
public class ManualIntegrationTest {
    ...
    //@Test
    public void testSchedulerNotSpecified() throws StreamingQueryException {
        fullScanTest(null);
    }

    //@Test
    public void testBatchScheduler() throws StreamingQueryException {
        fullScanTest("BatchScheduler");
    }


    //@Test
    public void testNoOpScheduler() throws StreamingQueryException {
        fullScanTest("NoOpScheduler");
    }

    private void fullScanTest(String scheduler) throws StreamingQueryException {
        ...
        if (scheduler == null) {
            df = spark
                    ...
                    // audit information
                    .option("TeragrepAuditQuery", "index=voyager-data")
                    ...
                    .option("kafka.bootstrap.servers", "kafkadev01.xnet.fi:9092,kafkadev02.xnet.fi:9092,kafkadev03.xnet.fi:9092")
                    .option("kafka.sasl.mechanism", "PLAIN")
                    .option("kafka.security.protocol", "SASL_PLAINTEXT")
                    .option("kafka.sasl.jaas.config", "org.apache.kafka.common.security.plain.PlainLoginModule required username=\"superuser\" password=\"SuperSecretSuperuserPassword\";")
                    .load();
        } else {
            df = spark
                    .readStream()
    ...
    private long preloadS3Data() throws IOException {
        ...
        TreeMap<Long, Result<Record10<ULong, String, String, String, String, Date, String, String, ULong, ULong>>> virtualDatabaseMap = mockDBData.getVirtualDatabaseMap();

        for (Map.Entry<Long, Result<Record10<ULong, String, String, String, String, Date, String, String, ULong, ULong>>> entry : virtualDatabaseMap.entrySet()) {
            Iterator<Record10<ULong, String, String, String, String, Date, String, String, ULong, ULong>> it = entry.getValue().iterator();
            while (it.hasNext()) {
                // id, directory, stream, host, logtag, logdate, bucket, path, logtime, filesize
                Record10<ULong, String, String, String, String, Date, String, String, ULong, ULong> record10 = it.next();
