...
public class ElementConditionTest {
    ...
    public void testStreamTags() {
        final List<String> streamTags = Arrays.asList("index", "host", "sourcetype");
        int loops = 0;
        for (String tag : streamTags) {
            final Element element = document.createElement(tag);
            element.setAttribute("value", "1000");
            element.setAttribute("operation", "EQUALS");
            final Condition condition = new ElementCondition(element, streamConfig).condition();
            Assertions.assertTrue(condition.toString().contains("1000"));
            loops++;
        }
        Assertions.assertEquals(loops, streamTags.size());
    }

    @Test
    public void testProvidedElementMissingValue() {
        final Element element = document.createElement("test");
        element.setAttribute("operation", "EQUALS");
        final ElementCondition elementCondition = new ElementCondition(element, config);
        final ElementCondition streamElementCondition = new ElementCondition(element, streamConfig);
    ...
    public void testIsIndexStatement() {
        final Element element = document.createElement("indexstatement");
        element.setAttribute("value", "searchTerm");
        element.setAttribute("operation", "EQUALS");
        final Element element2 = document.createElement("index");
        element2.setAttribute("value", "searchTerm");
        element2.setAttribute("operation", "EQUALS");
        final ElementCondition condition = new ElementCondition(element, config);
        Assertions.assertTrue(condition.isBloomSearchCondition());
        element.setAttribute("operation", "NOT_EQUALS");
        final ElementCondition condition2 = new ElementCondition(element, config);
        Assertions.assertFalse(condition2.isBloomSearchCondition());
        final ElementCondition condition3 = new ElementCondition(element, streamConfig);
        Assertions.assertFalse(condition3.isBloomSearchCondition());
        final ElementCondition condition4 = new ElementCondition(element2, streamConfig);
    ...
    public void testTimeQualifiers() {
        final List<String> tags = Arrays.asList("earliest", "latest", "index_earliest", "index_latest");
        int loops = 0;
        for (String tag : tags) {
            final Element element = document.createElement(tag);
            element.setAttribute("value", "1000");
    ...
    public void invalidElementNameTest() {
        ...
        final Element element2 = document.createElement("hostindex");
    ...
    public void equalsTest() {
        final Element element = document.createElement("index");
        element.setAttribute("value", "f17");
        element.setAttribute("operation", "EQUALS");
        final ElementCondition eq1 = new ElementCondition(element, config);
        eq1.condition();
        final ElementCondition eq2 = new ElementCondition(element, config);
    ...
    public void notEqualsTest() {
        ...
        element.setAttribute("value", "f17");
        element.setAttribute("operation", "EQUALS");
        final Element anotherElement = document.createElement("index");
        anotherElement.setAttribute("value", "f11");
        anotherElement.setAttribute("operation", "EQUALS");
        final ElementCondition eq1 = new ElementCondition(element, config);
        final ElementCondition notEq = new ElementCondition(anotherElement, config);
        final ElementCondition notEq2 = new ElementCondition(element, streamConfig);
    ...
    public void notEqualsDifferentConfigTest() {
        ...
        element.setAttribute("value", "f17");
        element.setAttribute("operation", "EQUALS");
        final MockConnection conn = new MockConnection(ctx -> new MockResult[0]);
        final DSLContext ctx = DSL.using(conn);
        final ConditionConfig cfg1 = new ConditionConfig(ctx, false);
        final ConditionConfig cfg2 = new ConditionConfig(ctx, true);
        final ElementCondition eq = new ElementCondition(element, cfg1);
        final ElementCondition notEq = new ElementCondition(element, cfg2);
    ...
    public void notEqualsDifferentConnectionTest() {
        ...
        element.setAttribute("operation", "EQUALS");
        final MockConnection conn = new MockConnection(ctx -> new MockResult[0]);
        final MockConnection conn2 = new MockConnection(ctx -> new MockResult[0]);
        final DSLContext ctx = DSL.using(conn);
        final DSLContext ctx2 = DSL.using(conn2);
        final ConditionConfig cfg1 = new ConditionConfig(ctx, false);
        final ConditionConfig cfg2 = new ConditionConfig(ctx2, false);
        final ElementCondition eq = new ElementCondition(element, cfg1);
        final ElementCondition notEq = new ElementCondition(element, cfg2);
        Assertions.assertNotEquals(eq, notEq);
    }
    ...
    public void testHashCode() {
        ...
        element.setAttribute("value", "f17");
        element.setAttribute("operation", "EQUALS");
        final Element element2 = document.createElement("source");
        element.setAttribute("value", "f17");
        element.setAttribute("operation", "EQUALS");
        final ElementCondition eq1 = new ElementCondition(element, config);
        final ElementCondition eq2 = new ElementCondition(element, config);
        final ElementCondition notEq = new ElementCondition(element2, config);
    ...
    public void equalsHashCodeContractTest() {
        ...
        final ValidElement validElement = new ValidElement(element);
        final ValidElement validElement2 = new ValidElement(element2);
